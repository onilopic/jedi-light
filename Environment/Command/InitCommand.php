...
namespace Prokl\JediLight\Environment\Command;
...
use Exception;
use Prokl\JediLight\Application\Command\Command;
use Prokl\JediLight\Application\Exception\BitrixException;
use LogicException;
            ...
            foreach ($moduleOptions as $code => $value) {
                if (is_array($value)) {
                    if (isset($value['value']) && isset($value['siteId'])) {
                        ...
                        $output->writeln(
                            '<error>Invalid option for module "' . $module . '" with code "' . $code . '"</error>'
                        );
                    }
                } else {
